...
\begin{example}
	...
	\begin{align*}
		\langle au + bv, w \rangle &= (au_1 + bv_1) w_1 + (au_2+bv_2) w_2 \\
					   ...
					   &= a(u_1w_1 + u_2w_2) + b(v_1v_2 + v_2w_2) \\ 
...
\begin{example}[Tax-cab Metric]
	In $\R^2$, the taxi-cab distance between $p=(p_1,p_2)$ and $q=(q_1,q_2)$ is
	 $|q_1 - p_2| + |q_2-p_2|$. It is easy to verify that this is a metric.
\end{example}
...
\begin{definition}[Open sets of $\R^n$ ]
	A subset $U \in \R^n$ is open if for every point $x \in U$, there exists $r > 0$ such that the open ball $B_r(x)$ is contained in $U$.
\end{definition}
